...
(async () => {
  ...
  try {
    ...
    const tree = await a.eval(`(() => { const spawn = [...document.querySelectorAll('#conv details.tool')].find(t => t.dataset.name === 'collab:spawn_agent');
    ...
    check('native child nested under the spawning tool call, grandchild under the child, with their own output', tree.spawn && tree.child === 'child task' && (tree.childText || '').trim() === 'child output' && tree.grand === 'grandchild task', tree);
    ...
    await clickIn(c, '#more'); await delay(300); await clickIn(c, '#tab-log');
    ...
    await clickIn(c, '#more'); await delay(300); await clickIn(c, '#tab-conv');
    check('Event log (… menu) lists the raw events; conversation returns', logRows > 5 && await c.eval(`!document.getElementById('conv').hidden`), logRows);
