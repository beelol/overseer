...
echo done`) });
    ...
    // The chat scrolls inside #scroll (the header and composer stay put).
    await out1.waitFor(`document.getElementById('scroll').scrollHeight > document.getElementById('scroll').clientHeight + 600`, 20000);
    await out1.eval(`document.getElementById('scroll').scrollTop = 400; const p = document.getElementById('prompt'); p.value = 'unsent follow-up draft'; p.dispatchEvent(new Event('input'));`);
    await delay(800);
    before.outputY = await out1.eval(`document.getElementById('scroll').scrollTop`);
    ...
    const verify = async (label, cdpNow) => {
      ...
      const expected = [/Review: R1 long review/, /Review: R2 other repo/, /^R1 long review/, /^R2 other repo/, /^R3 removed later/];
      ...
      await clickTab(/^R1 long review/);
      const ov = await outputFrame(t1.run.id);
      await delay(1200);
      const o = await ov.eval(`({ y: document.getElementById('scroll').scrollTop, draft: document.getElementById('prompt').value })`);
