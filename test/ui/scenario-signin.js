...
(async () => {
  ...
  try {
    ...
    check('a missing login is shown in Accounts and the account tile is disabled with the reason', /signed out/.test(missing || '') && tile?.disabled && /Sign in first/.test(tile.why || ''), { missing, tile });
    ...
    const signed = await refreshUntil('Claude work', /^max · [0-9a-f]{8}/);
    check('sign in through the UI', /^max · [0-9a-f]{8}/.test(signed || ''), signed);
    ...
    const again = await refreshUntil('Claude work', /^max · [0-9a-f]{8}/);
    ...
    check('"Sign in again" reauthenticates only that account and the follow-up then succeeds', /^max · [0-9a-f]{8}/.test(again || '') && ok.status === 'completed' && hello.some(t => t.includes('claudia-again')), { again, status: ok.status, hello });
