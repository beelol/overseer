...
(function () {
  ...
  function create(host, { post, onStarted }) {
    ...
    const more = ui.iconButton('settings', 'More options', { cls: 'sm', action: 'composer-more' }); more.setAttribute('aria-haspopup', 'menu');
    const start = ui.iconButton('arrow-up', 'Start agent', { cls: 'primary send', shortcut: 'Enter' }); start.id = 'start';
    row.append(chips, more, start);
    box.append(task, generic, row);
    ...
    function render() {
      ...
      setChip(agentChip, ui.harnessMark(form.harness, 14), agentLabel, [hx && `${ui.HARNESS[hx.harness]} ${hx.version || ''}`, a && `${a.name}: ${a.signedIn ? 'signed in' + (a.plan ? ' · ' + a.plan : '') : 'not signed in'}`].filter(Boolean).join('\n'));
    ...
    function validate() {
      const p = problem();
      note.replaceChildren(); note.className = 'composer-note' + (p.text && !p.soft ? ' error' : '');
      if (p.text) {
    ...
    function menuAgent() {
      ...
      for (const hx of data.harnesses) {
        ...
        for (const a of accts) items.push({ label: a.name, logo: ui.harnessMark(hx.harness, 14), hint: a.signedIn ? (a.plan || '') : 'signed out', checked: form.harness === hx.harness && form.account === a.id,
    ...
    function go() {
      ...
      post({ type: 'start', form: { ...form, prompt: task.value, program: program.value.trim(), args: args.value.trim() || '[]' } });
    }
    return {
      open(opts = {}) {
      ...
      data(d) {
        ...
        if (!d.harnesses.some(h => h.harness === form.harness && h.installed)) form.harness = (d.harnesses.find(h => h.installed && h.harness !== 'generic' && h.harness !== 'codex-app') || {}).harness;
