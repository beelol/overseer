...
rec(8, "Local access boundary", "partial", commit="8890f1d", date="2026-09-25",
    proven="owner-only socket (0600) and directory (0700); a connection from a uid other than the owner is refused without a reply, nothing runs, and the daemon logs `rejected connection from uid …` (test-only owner override); untrusted workspaces cannot launch; malformed requests cannot run shell fragments",
    deferred="a connection attempt from a real second macOS user (needs the owner's password for sudo)",
...
4. Packaged UI [trust](evidence/ui/trust/): an untrusted window (VS Code Restricted Mode via `security.workspace.trust.emptyWindow=false`) — the trust editor shows "You are in Restricted Mode", **Overseer: New Task** is not offered in the command palette, the Agents view explains that launching requires trust, and no task exists afterwards.""",
    ...
    blocker="Owner action: run `sudo -u nobody \"$HOME/.vscode/extensions/beelol.overseer-0.1.0/bin/overseerd-darwin-arm64\" ctl hello` (with `OVERSEER_HOME=\"$HOME/Library/Application Support/Overseer\"`) and confirm it fails (permission denied or no reply) and the daemon log shows `rejected connection from uid`.",
