...
| T-19 Attention | `tests/control.rs` `t19_…` | checked in the test (pty output) | Bell and "◆ Asks permission needs you — press w"; title "Overseer · 1 needs you · 2 active"; the real binary writes the title escape and a standalone bell. |
| T-20 Cleanup | `tests/control.rs` `t20_…` | [t20-cleanup](t20-cleanup.txt) | "1 uncommitted file will be LOST: notes-draft.txt"; `n` keeps it, `y` removes the worktree; the branch remains. |
| T-21 Stop and start | `tests/control.rs` `t21_…` | [t21-stop-all](t21-stop-all.txt) | Prompt names both agents; the daemon exits and is not respawned for 2.5 s; `r` restarts it in the test's home and both show as interrupted. |
| T-22 Open PR | `tests/control.rs` `t22_…` | [t22-open-pr](t22-open-pr.txt) | Refused while running; the confirmation names repository, branches, remote and gh; pushed branch = worktree HEAD; `gh pr create --repo test-owner/pr-demo --head overseer/add-notes --base main --title "Add notes"` with the generated body; `pull_request` event #7; `main` unchanged. |
