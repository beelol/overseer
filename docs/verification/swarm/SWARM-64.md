...
Actual: the first versioned backend is Catalog v1 for S3. Its 24 TypeScript routes provide tied sort keys and insert-between-page behavior. `node fixtures/swarm/catalog-v1/replay-backend.mjs` proves the offset baseline and timestamp-only cursor fail, while the tuple cursor passes. `cargo test --offline -p overseerd --test swarm_scenarios -- --ignored` replays a contract revision, one stale worker-result redirect, 26 isolated patch integrations and a combined checker that fails after four modules and passes after all 24. A second, non-ignored test uses the same manifest to admit four modules, then eight after first-wave acceptance, hold admission at eight pending reviews, resume below four, and accept all 24 modules. [S3](S3.md) records the exact fixture, commands and limitations.

Remaining: S3's director-driven adaptive choice and the combined admission-plus-integration trace, scope narrowing, conflict/fault variants, final artifact view and live communication path have not been replayed. S0–S2 and S4–S5 lack complete versioned backends and scenario traces. No RFC checkbox is checked.
