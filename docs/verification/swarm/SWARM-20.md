...
Global shutdown fixture (`ad6459b`): `daemon_stop_all_cancels_swarm_without_a_supervised_worker` first failed because `daemon.stop_all` returned no Swarm IDs and left its queued job runnable. The daemon now records Stop for every nonterminal category under the launch lock before interrupting processes, including categories with no Overseer worker run. On restart the queued job is cancelled and cannot be registered. The initial active-worker fixture launched one supervised local worker with a second job queued; global Stop interrupted the worker, cancelled the queued job, and kept one admitted attempt. The full offline Rust suite passed 190 tests with 11 ignored.

Control-loss regression (`11b36a5`): the active-worker fixture was tightened to remove its control socket while its process remained alive. Before the fix, `daemon.stop_all` stamped a false `interrupted` end time and treated the worker as exited. It now checks recorded supervisor and child PIDs when the socket cannot be reached, reports the surviving worker in `remaining`, and leaves recorded process runs for startup reconciliation. After daemon restart the worker remains active, the Swarm shows one unconfirmed exit, and the admitted job still has exactly one attempt. `cargo test --workspace --offline -q` passed 190 tests with 11 ignored; `git diff --check` passed. This is a local fixture and does not qualify native descendants or a live harness. SWARM-20 remains partial.

Follow-up guard (revision pending): the same fixture first observed `run.follow_up` successfully creating a new turn after the stopped worker exited, bypassing its Swarm attempt identity and admission. `start_turn_internal` now rejects ordinary follow-ups on a run linked to a Swarm attempt, which also covers merge-conflict handoff through that method. The focused fixture failed before the guard and passed afterward; `cargo test --workspace --offline -q` passed 190 tests with 11 ignored. This does not establish shared admission for ordinary non-Swarm follow-ups, which is still open.
