...
Global shutdown fixture (revision pending): `daemon_stop_all_cancels_swarm_without_a_supervised_worker` first failed because `daemon.stop_all` returned no Swarm IDs and left its queued job runnable. The daemon now records Stop for every nonterminal category under the launch lock before interrupting processes, including categories with no Overseer worker run. On restart the queued job is cancelled and cannot be registered. `daemon_stop_all_interrupts_linked_swarm_worker_and_preserves_attempt` launches one supervised local worker with a second job queued, then calls global Stop. The worker exits, the queued job is cancelled, and the admitted job still has exactly one attempt after daemon restart. Both focused fixtures passed; `cargo test --workspace --offline -q` passed 190 tests with 11 ignored; `git diff --check` passed. This adds a global control path but does not qualify native descendants or a live harness.
